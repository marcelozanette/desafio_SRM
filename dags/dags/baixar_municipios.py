...
def upload_to_minio(file_path, file_name):
    ...
    try:
        ...
        hook.load_file(
            filename=file_path,
            key=f"municipios_{ano_mes}/{file_name}",
...
def baixar_apenas_municipios():
    ...
    try:
        ...
        print("🔍 Iniciando busca por arquivos MUNICIPIOS...")
        while True:
            # Rola suavemente (19 vezes de 100px) para carregar o meio da lista
            for _ in range(19):
                driver.execute_script("""
                    var c = document.querySelector('#app-content') || document.querySelector('.files-list') || window;
                    c.scrollBy(0, 100);
                """)
                time.sleep(0.1) # Intervalo de 0.1s entre cada "clique" de scroll
            
            time.sleep(3) # Pausa para o site processar o que apareceu na tela


            # Captura todos os elementos 'MUNICIPIOS' carregados
            elementos = driver.find_elements(By.XPATH, "//tr[contains(@data-cy-files-list-row-name, 'Municipios')]")
            ...
            if not nome_alvo:
                print("🏁 Fim da lista. Todos os arquivos MUNICIPIOS foram processados.")
                break
...
with DAG(
    ...
    catchup=False,
    tags=['cnpj', 'municipios']
) as dag:

    PythonOperator(
        task_id='baixar_e_upload_minio',
        python_callable=baixar_apenas_municipios
    )
